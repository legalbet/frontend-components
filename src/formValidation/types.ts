...
export interface UseFormWithValidation<T extends Record<string, any>> {
  ...
  checkFieldValidity: (field: keyof T) => string[]; // Function to check field validity without setting errors
  isFormDisable: ComputedRef<boolean>; // check when to disable form button
